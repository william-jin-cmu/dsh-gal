#!/bin/bash
# Build the dsh-gal external plugin: compile src/ → lib/ (JS + declarations)
# with the dsh checkout's TypeScript. The plugin's node_modules holds symlinks
# into the dsh checkout so tsc type-checks against the same packages the
# running dsh ships. Requires `dsh` on PATH.
set -euo pipefail
...
link_pkg() {
  ...
  if [ ! -e "$target" ]; then
    echo "build: missing $2 in checkout" >&2
    exit 1
  fi
